...
for (analysis in analyses_made){
  print(analysis)
  dat.results = rbind(dat.results,invisible(process_results_linear(analysis,exposure,1,'_sensitivity.rds')))
}
...
dat.results %>%
  write.csv(paste0(cause.model.exploration.folder,'overall_dlnm_results_freq_lag01_sensitivity.csv'),row.names = FALSE)
