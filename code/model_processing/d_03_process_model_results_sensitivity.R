...
for (analysis in analyses_made){
  print(analysis)
  dat.results = rbind(dat.results,invisible(process_results_linear(analysis,exposure,6,'_sensitivity.rds')))
}
...
dat.results %>%
  write.csv(paste0(cause.model.exploration.folder,'overall_dlnm_results_freq_sensitivity.csv'),row.names = FALSE)
