...
aics = aics %>%
  mutate(variable='wbgtmax')

# # Save results as csv
readr::write_csv(aics,paste0(cause.model.exploration.folder,cause,'_aic_values_wbgtmax.csv'))

# plotting functions
source(paste0(functions.folder,'plotting_functions.R'))

# Plot full lag results of optimal version
pdf(paste0(cause.model.exploration.folder,cause,'_wbgtmax_var1.lag6.pdf'),paper='a4r',height=0,width=0)
