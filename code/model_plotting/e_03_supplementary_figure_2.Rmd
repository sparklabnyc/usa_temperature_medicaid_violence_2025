---
title: "Supplementary Figure 2 plot"
...
dat.results = read.csv(paste0(cause.model.exploration.folder,'overall_dlnm_results_freq_lag01.csv'))
```
...
dat.results = dat.results %>%
  ...
  mutate(group=case_when(
    ...
    mutate(analysis=case_when(
          ...
          analysis == "education1" ~ "1 (lowest education)",
          analysis == "education2" ~ "2",
          analysis == "education3" ~ "3",
          analysis == "education4" ~ "4",
          analysis == "education5" ~ "5 (highest education)",
...
# Split into dry and humid temperature
```{r}
dat.results.dry = dat.results %>%
  filter(exposure=='Dry temperature') %>%
  select(-exposure) %>%
  rename(central.est.dry=central.est,ll.est.dry=ll.est,ul.est.dry=ul.est)

dat.results.humid = dat.results %>%
  filter(exposure=='Humid temperature') %>%
  select(-exposure) %>%
  rename(central.est.humid=central.est,ll.est.humid=ll.est,ul.est.humid=ul.est)

dat.results.compare = left_join(dat.results.dry,dat.results.humid) %>%
  filter(exposure_change==exposure_change_chosen)
```

# Plot Supplementary Figure 2
```{r}
p = ggplot(data=dat.results.compare) +
    geom_point(aes(x=central.est.dry,y=central.est.humid,color=cause),size=2) +
    geom_errorbar(aes(x=central.est.dry,ymax=ul.est.humid,ymin=ll.est.humid,color=cause),
                  width=0,size=0.5) +
    geom_errorbar(aes(y=central.est.humid,xmax=ul.est.dry,xmin=ll.est.dry,color=cause),
                  width=0,size=0.5) +
    facet_wrap(~cause,scale='free') +
    geom_abline(linetype='dotted') +
    xlab("Dry temperature") + ylab('Humid temperature') +
    scale_x_continuous(labels=scales::percent_format(accuracy=1)) +
    scale_y_continuous(labels=scales::percent_format(accuracy=1)) +
    # coord_equal() +
    # scale_colour_manual(values=colors.exposures) +
    labs(colour="", shape="") +
    theme_bw() + theme(text = element_text(size = 10),legend.text=element_text(size=12),
...
pdf(paste0(supplementaryfigure2.folder,'supplementaryfigure2.pdf'),paper='a4r',height=0,width=0)
p
dev.off()
