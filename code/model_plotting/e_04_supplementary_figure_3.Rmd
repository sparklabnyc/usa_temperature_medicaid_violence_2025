---
title: "Supplementary Figure 3 plot"
...
dat.results = read.csv(paste0(cause.model.exploration.folder,'overall_dlnm_results_freq_lag01_sensitivity.csv'))
```
...
# Plot Supplementary Figure 3 (lag01)
```{r}
...
pdf(paste0(supplementaryfigure3.folder,'supplementaryfigure3.pdf'),paper='a4r',height=0,width=0)
print(p)
dev.off()
...
# Plot Supplementary Figure 3 option 2 (lag01)
```{r}
p2 = ggplot(data=dat.results %>% filter(exposure_change==exposure_change_chosen)) +
    geom_errorbar(aes(x=analysis,ymax=ul.est,ymin=ll.est,color=exposure),width=.5,size=0.5,position=position_dodge(0.7)) +
    geom_point(aes(x=analysis,y=central.est,color=exposure),size=2,shape=16,position=position_dodge(0.7)) +
    facet_grid(group~cause,scales="free", space ="free_y") +
    geom_hline(yintercept=0,linetype='dotted') +
    xlab("") + ylab('Cumulative percentage change in hospitalization rates\nfor 5°C increase') +
    scale_y_continuous(labels=scales::percent_format(accuracy=1)) +
    scale_colour_manual(values=colors.exposures) +
...
pdf(paste0(supplementaryfigure3.folder,'supplementaryfigure3.pdf_option2.pdf'),paper='a4',height=0,width=0)
print(p2)
dev.off()
